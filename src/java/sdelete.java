...
public class sdelete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            PreparedStatement ps=con.prepareStatement("delete  from sent where slno =?");
            ps.setInt(1,v);
            ps.executeUpdate();
         out.println ("<html><body><script>alert('Sucessfully Deleted');</script></body></html>");
         String msg="The mail has been removed";
         request.setAttribute("mssg",msg);
                RequestDispatcher rd=request.getRequestDispatcher("sent");
