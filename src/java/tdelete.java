...
public class tdelete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      Integer v=Integer.parseInt(request.getParameter("val"));
            ...
            PreparedStatement ps=con.prepareStatement("delete  from trash where slno =?");
            ps.setInt(1,v);
            ps.executeUpdate();
         out.println ("<html><body><script>alert('Sucessfully Deleted');</script></body></html>");
         String msg="The mail has been permanently deleted";
         request.setAttribute("mssg",msg);
                RequestDispatcher rd=request.getRequestDispatcher("trash");
                rd.include(request, response);
        } catch (Exception ex) {
            Logger.getLogger(tdelete.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
